...
from src.util.get_stock_data import get_stock_data
...
class AStockScreener:
    ...
    def process_stock(self, code, name, rsi_params=None):
        """处理单个股票的完整流程，使用独立的get_stock_data函数"""
        # 获取股票数据
        data = get_stock_data(
            code=code,
            name=name,
        ...
        # 检查数据是否可用于分析
        if '数据可用' in data.columns and not data['数据可用'].any():
            print(f"警告: 股票 {code} 的数据不可用于技术分析，跳过")
            return None
